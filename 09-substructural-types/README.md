...
```
A@r where r tracks resource usage

x:A@1, y:B@1 ⊢ ⟨x,y⟩ : (A ⊗ B)@1    -- Each resource used once
x:A@2 ⊢ ⟨x,x⟩ : (A ⊗ A)@2            -- Resource used twice
```
...
A@1 <: A@*  -- Linear resources can be used unrestrictedly
```
...
A@? -- Unknown resource annotation
cast : A@r → A@s  -- Runtime check for resource compatibility
```

### Dependent Substructural Types
```
Vec : Nat → Type → Resource → Type
append : ∀n m. Vec n A@r → Vec m A@r → Vec (n+m) A@r
```
